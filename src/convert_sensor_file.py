...
def extract_data(file_path: Path, input_filename: str) -> dict[str, list[np.ndarray]]: # type: ignore
    all_accel_data = []
    all_gyro_data = []
    ...
    pkl_filename = input_filename.replace('.mat', '.pkl')
    output_file_path = cfg.SENSOR_DATA / pkl_filename
...
if __name__ == "__main__":
    ...
    input_files = [
        'phys_cur_HC_merged.mat',
        'phys_cur_PD_merged.mat'
    ]
    
    for filename in input_files:
        file_path = cfg.SENSOR_DATA / filename
        
        if not file_path.exists():
            print(f"\nWARNING: Input file not found: {file_path}. Skipping.")
            continue
        extracted_data = extract_data(file_path, filename)
